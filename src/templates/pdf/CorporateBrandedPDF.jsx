...
function renderPdfSkills({ layout = 'columns2', skills, s, dm }) {
            ...
            <View style={{ flexDirection: 'column', gap: 6 }}>
                {active.map(({ key, label }) => (
                    <View key={key} style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                        <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 7, color: s.skillCat.color, textTransform: 'uppercase', letterSpacing: 0.8, width: 110, marginTop: 1 }}>{label}</Text>
                ...
                {active.map(({ key, label }) => (
                    <View key={key} style={{ flexDirection: 'row', alignItems: 'flex-start', width: '47%' }}>
                        ...
                        <Text style={{ fontFamily: 'Inter', fontSize: s.skillItem.fontSize, color: '#444', lineHeight: 1.5 }}>
                            <Text style={{ fontWeight: 700, color: '#111', fontSize: 7, textTransform: 'uppercase' }}>{label}: </Text>
                            {skills[key].map(skill => cleanAndCapitalizeSkill(skill)).join(', ')}
                        </Text>
                    </View>
                ))}
        ...
        skills: () => {
            ...
            return (
                <View key="skills" style={s.section} wrap={false}>
                    ...
                    {renderPdfSkills({ layout: career.skillsLayout || 'columns2', skills: positioned.skills, s, dm })}
                </View>
            )
        },
